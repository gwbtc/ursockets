...
import { Maximize2, Minimize2, Plus, Trash2, Split } from "lucide-react";
...
function PermsWrapper({
  ...
  return (
    <div className="perms-modal-content" style={{ padding: "20px", color: "#fff" }}>
...
function Composer({ isAnon }: { isAnon?: boolean }) {
  const { api, composerData, addNotification, setComposerData, setModal, contacts, profiles } =
    useLocalState((s) => ({
      api: s.api,
  ...
  // Autocomplete state
  const [mentionState, setMentionState] = useState<{ active: boolean; query: string; type: string; start: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const urls = words.filter(w => w.startsWith("http") || w.startsWith("blob:"));
  ...
  const checkMention = (text: string, cursor: number) => {
      ...
      // Match @... or ~... at the end of the string
  ...
  const getFilteredUsers = () => {
      ...
      if (contacts) {
          Object.entries(contacts).forEach(([ship, data]) => {
              ...
              if (ship.includes(q) || nick.toLowerCase().includes(q)) {
      ...
      if (profiles) {
          profiles.forEach((prof, key) => {
              ...
              if (!matches.find(m => m.id === key)) {
                  if (key.includes(q) || name.toLowerCase().includes(q)) {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    if (e.clipboardData.files.length > 0) {
      ...
      if (file.type.startsWith("image/")) {
        ...
        const newText = textBefore + (textBefore && !textBefore.endsWith(" ") ? " " : "") + url + " " + textAfter;
        
        setInput(newText);
        toast("Image pasted (local preview only - upload not implemented)", {
            icon: "⚠️",
            style: {
                borderRadius: '10px',
                background: '#333',
                 ...
                 if (our) {
                     ...
                     if ('ok' in res) {
                        ...
                        // Actually, let's try linear:
                         const feed = Object.values(res.ok.feed.feed).sort((a, b) => b.time - a.time);
                         if (feed[0] && feed[0].contents[0] && JSON.stringify(feed[0].contents).includes(part.slice(0, 20))) {
                             lastId = feed[0].id;
                         }
                     }
                 }
  ...
  return (
    ...
    >
      ...
      {isMaximized && (
          <button type="button" className="close-maximized" onClick={() => setIsMaximized(false)}>
              <Minimize2 size={20} />
          </button>
      )}
      ...
      <div className="composer-content">
        ...
        <div className="composer-input-row" style={{ position: 'relative' }}>
          {isThreadMode ? (
              <div className="thread-editor" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {threadParts.map((part, i) => (
                      <div key={i} style={{ display: 'flex', gap: '8px' }}>
                          ...
                          />
                          <button type="button" onClick={() => removeThreadPart(i)} style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer' }}>
                              <Trash2 size={16} />
                          </button>
                      </div>
                  ))}
                  <button type="button" onClick={addThreadPart} style={{ alignSelf: 'center', display: 'flex', alignItems: 'center', gap: '4px', background: 'transparent', border: '1px dashed #666', color: '#aaa', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' }}>
                          ...
                          {filteredUsers.map((user, i) => (
                              ...
                              >
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                                      <span style={{ fontWeight: 500, fontSize: '0.9em' }}>{user.display}</span>
                                      {user.sub !== user.display && <span style={{ fontSize: '0.75em', opacity: 0.8 }}>{user.sub}</span>}
                                  </div>
                              </div>
                          ))}
                      </div>
          ...
          <div className="composer-controls" style={{ display: 'flex', flexDirection: isMaximized ? 'row' : 'column', gap: '8px', marginTop: '8px' }}>
              <button
                type="button"
              ...
              {!isThreadMode && input.length > 280 && (
                  ...
                  >
                    <span style={{ fontSize: '12px', fontWeight: 'bold' }}>Split</span>
                  </button>
              )}
          </div>
          ...
          {isLoading ? (
            ...
            <button type="submit" disabled={!isThreadMode && !input.trim()} className="post-btn">
                    ...
                    <div key={p.url + i} className="preview-item">
                        ...
                        {p.type === "link" && (
                            <div className="link-preview">
                                ...
                                <span style={{fontSize: '0.8em', marginLeft: '4px'}}>{p.url.length > 30 ? p.url.slice(0, 27) + "..." : p.url}</span>
                            </div>
                        )}
                    </div>
                ))}
