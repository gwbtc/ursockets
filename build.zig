...
const dependencies = [_]RepoImport{
    ...
    .{
        ...
        .prefix = "desk",
        .paths = &.{
            "mar/contact/action-0.hoon",
            "mar/contact/action-1.hoon",
            "mar/contact/action.hoon",
            "mar/contact/book-0.hoon",
            "mar/contact/book.hoon",
            "mar/contact/directory-0.hoon",
            "mar/contact/directory.hoon",
            "mar/contact/news.hoon",
            "mar/contact/page-0.hoon",
            "mar/contact/page-1.hoon",
            "mar/contact/page.hoon",
            "mar/contact/response-0.hoon",
            "mar/contact/rolodex.hoon",
...
pub fn build(b: *std.Build) void {
    ...
    const named_build = b.step("build", "Build /dist from /app and dependencies");
...
fn buildDesk(step: *std.Build.Step, allocator: std.mem.Allocator, copy_target: ?[]const u8) !void {
    if (!pathExists("app")) {
        return step.fail("/app directory not found", .{});
    }

    std.debug.print("Creating /dist directory...\n", .{});
    try recreateDir("dist");

    std.debug.print("Copying /app to /dist...\n", .{});
    try copyDirContents(allocator, "app", "dist");
